...
import { Offer, PricePointDetails } from '../state/types'
...
export interface PricePointDummy
  extends Omit<PricePointDetails, 'type' | 'volume' | 'totalVolume'> {
  type: Offer | null
  volume: number | null
...
const addClearingPriceInfo = (
  price: number,
  pricePointsDetails: (PricePointDetails | PricePointDummy)[],
): (PricePointDetails | PricePointDummy)[] => {
  const pricePointBottom: PricePointDummy = {
    type: null,
    volume: null,
    totalVolume: null,
    price,
  ...
  const valueYofBids = pricePointsDetails.map((y) => Math.max(y.bidValueY || 0, y.askValueY || 0))
  const maxValueYofBid = Math.max(...valueYofBids)
  const pricePointTop: PricePointDummy = {
    type: null,
    volume: null,
    totalVolume: null,
    price,
    ...
    (acc, pricePoint, index) => {
      ...
      if (userOrder && userOrder == pricePoint) {
        ...
        const pricePointDetails: PricePointDetails = {
          ...
          askValueY: null,
          bidValueY: null,
          newOrderValueY: (bidValueY || 0) + volume,
          clearingPriceValueY: null,
        }
        acc.points.push(pricePointDetails)
...
export const processOrderbookData = ({
  ...
  quoteToken,
  userOrder,
}: ProcessRawDataParams): (PricePointDetails | PricePointDummy)[] => {
  if (!data) {
    return []
  }
  try {
    ...
    const bids = processData(data.bids, userOrder, max_value, min_value, Offer.Bid)
